...
public class Paging {
	public Map<String , Integer> paging(int currentPage, int totalCnt, int BOARD_LIMIT, int PAGE_LIMIT){
		...
		/*
		 * 유기적 페이징 
		 * 1. @RequestParam(value="page", defaultValue="1") int page ==> 전달 받는 인자에 이걸 추가하세요 
		 * 
		 * 2. int currentPage = page; int totalCnt= service.selectCount();
		 * Map<String, Integer> map= new Paging().paging(currentPage, totalCnt,
		 * BOARD_LIMIT, PAGE_LIMIT); mv.addObject("pageInfo", map); ==> 함수 내부에 넣어 주세요
		 */
